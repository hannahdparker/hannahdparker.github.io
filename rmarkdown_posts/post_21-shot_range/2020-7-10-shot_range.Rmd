...
team_pts_by_location <- read_csv("../../data/post21_shot-range/team_pts_by_location.csv") %>%
  mutate_at(.vars = vars(fgm_restricted_area_pts_percentage:ftm_pts_percentage),
            .funs = list(~ round(., 4)))

new_values_player <- read_csv("../../data/post21_shot-range/player_new_values.csv") %>%
    mutate_at(.vars = vars(old_ppg:ppg_difference),
            .funs = list(~ round(., 2))) %>%
  mutate_at(.vars = vars(ppg_percent_inc),
            .funs = list(~ round(., 4)))
```
...
```{r, echo=F}
DT::datatable(team_pts_by_location,
              options = list(dom = 't', pageLength = 30),
              rownames = F,
              colnames = c("team", "restricted_area", "in_the_paint",
                           "mid_range", "left_corner_three",
                           "right_corner_three", "above_the_break_three",
                           "free_throw"))
```
...
```{r, echo=F}
DT::datatable(new_values_player,
              options = list(dom = 'ftp', pageLength = 10),
              rownames = F,
              colnames = c("player", "team", "ppg", "adjusted_ppg",
                           "ppg_increase", "ppg_percent_increase"))
```
