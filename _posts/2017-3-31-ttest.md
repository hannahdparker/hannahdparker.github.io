...
the null, but if it's not we fail to reject it. 
...
we'd see a this result or a more extreme one given that our null hypothesis 
is true. It's not a measure of effect size. Make sure to include things like 
confidence intervals, which shows a range as to where the true value lies, as 
a way to back up your claims, not just the p-value.
